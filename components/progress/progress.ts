...
class Progress extends Base {
    ...
    setObserve({ target, attrs, svg, line, text }: any) {
        ...
        runIFELSE(new Set([
            [has(attrs, 'percentage'), () => {
                ...
                text &&( text.innerText = percentage)
                if (type == 'line') {
                ...
                let style = `stroke-dasharray:${target.perimeter * rate * (parseInt(attrs['percentage']) / 100)}px,${target.perimeter}px ;
                ...
                if(parseInt(attrs['percentage']) <= 0) {
                    sto(() => {
                        svg?.path2?.setAttribute('style', style + 'rgb(229, 233, 242);')
                    }, 90)
                }
            }],
            [has(attrs, 'color'), () => {
            ...
            [has(attrs, 'status') || attrs?.['classname'], () => {
    ...
    setOnce({ target, attrs }: any) {
        ...
        if (type == 'line') {
            ...
            path1?.setAttribute('style', style + `stroke: rgb(229, 233, 242);`)
